...
(function () {
  ...
  // ============================================================================
  // AUTO-MAP sucursal→CUIT desde la hoja de facturación/envío del propio Excel.
  // Para grupos NO hardcodeados (Poy, Megashop, Primer Precio, etc.): el Excel
  // trae una hoja "DATOS FACTURACION" / "DATOS ENVIO" que mapea cada sucursal a
  // su razón social + CUIT + domicilio de entrega. La parseamos y resolvemos el
  // cliente por CUIT (mismo lookup que los grupos hardcodeados). Soporta formato
  // TABLA (columnas LOCAL/CUIT) y BLOQUES DE TEXTO (celdas con "CUIT: ..."). Si
  // una sucursal no matchea con confianza queda sin resolver → el operador elige
  // el cliente a mano (se mantienen los controles actuales).
  // ============================================================================
  function _sucNorm(s) {
  ...
  var _SUC_STOP = { de: 1, la: 1, el: 1, los: 1, las: 1, "y": 1, del: 1, mega: 1, shop: 1, local: 1, sucursal: 1, suc: 1, primer: 1, precio: 1, nuevo: 1, transporte: 1 };
  function _sucTokens(s) {
    // Mantener los dígitos como tokens distintivos: así "MORENO 1" vs "MORENO 2"
    // o "SAN RAFAEL 2" vs "SAN RAFAEL" no se confunden.
    return _sucNorm(s).split(" ").filter(function (t) {
      return /^\d+$/.test(t) ? true : (t.length >= 3 && !_SUC_STOP[t]);
    });
  }
  function _onlyDigits(s) { return String(s == null ? "" : s).replace(/[^\d]/g, ""); }

  // Extrae [{ local, razon, cuit, delivery }] desde la hoja de facturación/envío.
  function parseFacturacionSheet(sheets) {
    var out = [];
    if (!sheets || !sheets.length) return out;
    // 1) Hoja candidata: por nombre (factur/envio/entrega/datos) o, si no, la
    //    primera hoja cuyo contenido mencione "CUIT".
    var cand = null;
    for (var i = 0; i < sheets.length; i++) {
      if (/factur|envio|entrega|datos/.test(_sucNorm(sheets[i].name))) { cand = sheets[i]; break; }
    }
    if (!cand) {
      for (var j = 0; j < sheets.length; j++) {
        var aoaj = sheets[j].aoa || [];
        var hit = aoaj.some(function (row) {
          return (row || []).some(function (v) { return /cuit/i.test(String(v == null ? "" : v)); });
        });
        if (hit) { cand = sheets[j]; break; }
      }
    }
    if (!cand || !cand.aoa) return out;
    var aoa = cand.aoa;
    // 2) Intento TABLA: fila header con columnas LOCAL y CUIT.
    var hdr = -1, cLocal = -1, cCuit = -1, cRazon = -1, cDom = -1;
    for (var r = 0; r < Math.min(aoa.length, 40); r++) {
      var row = aoa[r] || [];
      var li = -1, ci = -1, ri = -1, di = -1;
      for (var c = 0; c < row.length; c++) {
        var n = _sucNorm(row[c]);
        // Un header real es una etiqueta corta; una celda-bloque de texto (que
        // contiene "LOCAL"/"CUIT" dentro del párrafo) NO debe tomarse como header.
        if (!n || n.length > 24) continue;
        if (li < 0 && /\blocal\b|sucursal/.test(n)) li = c;
        if (ci < 0 && /\bcuit\b/.test(n)) ci = c;
        if (ri < 0 && /razon social/.test(n)) ri = c;
        if (di < 0 && /domicilio.*entrega|entrega/.test(n)) di = c;
      }
      // Exigir LOCAL y CUIT en columnas DISTINTAS → tabla real (si no, es blob).
      if (li >= 0 && ci >= 0 && li !== ci) { hdr = r; cLocal = li; cCuit = ci; cRazon = ri; cDom = di; break; }
    }
    if (hdr >= 0) {
      for (var r2 = hdr + 1; r2 < aoa.length; r2++) {
        var row2 = aoa[r2] || [];
        var local = String(row2[cLocal] == null ? "" : row2[cLocal]).trim();
        var cuit = _onlyDigits(row2[cCuit]);
        if (!local || cuit.length < 10) continue;
        out.push({
          local: local, cuit: cuit,
          razon: cRazon >= 0 ? String(row2[cRazon] == null ? "" : row2[cRazon]).trim() : "",
          delivery: cDom >= 0 ? String(row2[cDom] == null ? "" : row2[cDom]).trim() : "",
        });
      }
      if (out.length) return out;
    }
    // 3) Fallback BLOQUES DE TEXTO: celdas que contienen "CUIT: ...".
    aoa.forEach(function (row) {
      (row || []).forEach(function (v) {
        if (typeof v !== "string" || !/cuit/i.test(v)) return;
        var cuit = _onlyDigits((v.match(/cuit[:\s]*([\d.\-]{10,16})/i) || [])[1]);
        if (cuit.length < 10) return;
        var local = ((v.match(/local\s*[:\-]?\s*([^\n]+)/i) || [])[1] || "").trim().replace(/["“”]/g, "");
        var razon = ((v.match(/raz[oó]n\s*social[:\s]*([^\n]+)/i) || [])[1] || "").trim();
        var delivery = ((v.match(/domicilio\s*de?\s*entrega[:\s]*([^\n]+)/i) || [])[1] || "").trim();
        out.push({ local: local, cuit: cuit, razon: razon, delivery: delivery });
      });
    });
    return out;
  }

  // Matchea el header de una columna-sucursal contra las entradas parseadas.
  // Devuelve { groupName, cuit, label, delivery, autoParsed:true } | null.
  function resolveBranchAuto(headerText, entries) {
    if (!entries || !entries.length) return null;
    var qt = _sucTokens(headerText);
    if (!qt.length) return null;
    var qset = {};
    qt.forEach(function (t) { qset[t] = 1; });
    var best = null, bestSc = -1e9, bestOv = 0;
    entries.forEach(function (e) {
      var et = _sucTokens(e.local);
      if (!et.length) return;
      var ov = 0, extra = 0;
      et.forEach(function (t) { if (qset[t]) ov++; else extra++; });
      // El overlap manda; "extra" (tokens del local que no están en el header)
      // desempata hacia el local más específico: para el header "SAN RAFAEL 2"
      // gana "SAN RAFAEL 2" sobre "SAN RAFAEL".
      var sc = ov * 3 - extra;
      if (ov >= 1 && sc > bestSc) { bestSc = sc; best = e; bestOv = ov; }
    });
    if (best && bestOv >= 1) {
      return {
        groupName: "Auto (hoja facturación)",
        cuit: best.cuit,
        label: best.razon || best.local,
        delivery: best.delivery || best.local,
        autoParsed: true,
      };
    }
    return null;
  ...
  var BRANCH_EXCLUDE = /\b(bonif|dto|descuento|imp\w*|iva|rec\w*|retorno|sub.?total|total|pendiente|stock|saldo|venta|vta|precio|margen|markup|ganancia|costo)\b/i;
  ...
  function createCardInstance(idx, cardRoot) {
    ...
    var state = {
      ...
      branchCustMap: {},
      autoBranchEntries: [], // entradas parseadas de la hoja de facturación/envío
    ...
    async function handleFile(file) {
      ...
      try {
        ...
        // Auto-map: para branches sin grupo hardcodeado, resolver contra la hoja
        // de facturación/envío del propio Excel (Poy, Megashop, Primer Precio…).
        state.autoBranchEntries = parseFacturacionSheet(parsed.sheets);
        state.cols.branchCols.forEach(function (bi) {
          if (state.branchCustMap[bi]) return; // ya resuelto por grupo hardcodeado
          var ainfo = resolveBranchAuto(state.cols.headers[bi], state.autoBranchEntries);
          if (ainfo) {
            state.branchCustMap[bi] = ainfo;
            if (!state.detectedGroupName) state.detectedGroupName = "Auto (hoja facturación)";
          }
        });
    ...
    function renderResult() {
      ...
      if (sheetSel) {
        sheetSel.addEventListener("change", async function () {
          ...
          // Auto-map (reusa entradas ya parseadas de la hoja de facturación)
          if (state.autoBranchEntries && state.autoBranchEntries.length) {
            state.cols.branchCols.forEach(function (bi) {
              if (state.branchCustMap[bi]) return;
              var ainfo = resolveBranchAuto(state.cols.headers[bi], state.autoBranchEntries);
              if (ainfo) {
                state.branchCustMap[bi] = ainfo;
                if (!state.detectedGroupName) state.detectedGroupName = "Auto (hoja facturación)";
  ...
  // Export de helpers puros para reuso desde el módulo del vendedor
  // (vendor-import-excel.js) — mismo parseo/match/submit, sin duplicar lógica.
  window.ExcelKrikosCore = {
    parseWorkbook: parseWorkbook,
    analyzeColumns: analyzeColumns,
    isJunkRow: isJunkRow,
    matchRowToProduct: matchRowToProduct,
    parseFacturacionSheet: parseFacturacionSheet,
    resolveBranchAuto: resolveBranchAuto,
    resolveBranchCustomer: resolveBranchCustomer,
    loadCustomerByCuit: loadCustomerByCuit,
    sendToSheetsWithRetry: sendToSheetsWithRetry,
    sendToEntregas: sendToEntregas,
  };
})();
