...
- **El rol VENDEDOR ya existía y no es un rol aparte: es un cliente sintético.** Un vendedor es una fila de `customers` con `cod_cliente = 10000 + vend` y CUIT sintético `99xxxxxxxxx` (`generate_synthetic_vendor_cuit`); entra por `mayorista.html` con usuario + PIN como cualquier cliente y su cartera son las filas de `user_customer_links`, que el trigger `fn_autolink_vendor` engancha solo cuando aparece un cliente con su mismo `vend`. Al 18/9/2026 hay **13 vendedores con login** (Andrés 160 clientes, Borisosky 79, Monin 77, Cagnolo 60, Audisio 45, Zaffaroni 41, Schinder 34, Antonelli 22, Serra 19, Omar 8, La Bianca 7, Horizonte 6, Thomas LK 5). En el frontend lo resuelven `isActualVendor()` (perfil propio 100XX o Loekemeyer SRL cod 1), `isVendorOwnMode()` (vendedor mirando lo suyo) y `isVendorProfileBrowseMode()` (todavía no eligió cliente: ve solo precio de lista y no puede cargar al carrito). **Antes de agregarle algo al vendedor, mirar esto: no hay que crear ningún rol.**
- **`get_mis_clientes_inactivos(p_meses)` alimenta la card "Clientes que no están comprando"** del perfil del vendedor (`sql/vendedor_inactivos.sql`, `.vinact-*` en `css/styles.css`, módulo `loadVendorInactivosUI` en `script.js`). Devuelve la cartera propia con última factura, importe de esa factura, meses sin comprar, localidad, WhatsApp y si el cliente le sigue comprando a Chef. **El filtro "yo soy vendedor" no es decorativo**: la primera versión armaba la cartera con `user_customer_links OR mismo vend` sin exigirlo, y medido daba **37 clientes ajenos a un cliente mayorista común logueado** (razón social, localidad, última compra e importe). Verificado después del fix: Andrés 97 filas a 6 meses, Cagnolo 37, cliente común 0, sin sesión 0; 147 ms con la cartera más grande. `EXECUTE` revocado a `PUBLIC`/`anon`.
- **El ACUERDO (qué deja cada cliente) tiene un orden de descuentos que no es libre** (Thomas, 18/9/2026): el **dto de volumen va sobre la lista**, el **25% de pago sobre ese saldo**, el **2% del cotizador sobre el siguiente** —eso es el **cheque**— y recién ahí se restan **flete (1%) y comisión, los dos SOBRE EL CHEQUE y no encadenados entre sí**. Verificado contra su planilla con California (cod 588, dto 3%, comisión 7%): cheque 107,66 · flete −1,08 · comisión −7,54 · **recibo 99,04, acuerdo −0,96, factor 1,525**. **`get_acuerdo_vendedores` encadena flete y comisión** (99,12, −0,88): 0,08 de más por cliente, siempre a favor, así que las dos no dan idéntico. **Y los descuentos se dan SIN COMA**: el tope útil es el entero de abajo (2,06% → **2%**).
- **`get_acuerdo_cliente(p_cod)` + `acuerdo_parametros` alimentan la tarjeta "Acuerdo" de la Ficha de Cliente** (`sql/acuerdo_cliente.sql`, `.fc-acu-*` en `css/admin.css`, dentro de `fcRender` en `admin.js`). Devuelve cheque, recibo, acuerdo, factor, **`dto_max`** (cuánto descuento admite el cliente sin bajar de 100) y una **simulación por comisión**: bajando la comisión del vendedor de 7% a 4%, el tope de descuento del cliente sube de **2% a 5%** — con eso California pasaría de −0,96 a **+2,27**. Es casi 1 a 1: cada punto de comisión que se baja habilita ~1,03 de descuento. Los cinco parámetros (índice 151, pago 25%, cotizador 2%, flete 1%, piso 100) viven en `acuerdo_parametros` —una fila, RLS de admin— para cambiarlos sin tocar código. **No toca `get_ficha_cliente`** (8.900 caracteres): el frontend llama a las dos en paralelo y, si el acuerdo falla, la ficha se muestra igual. Chequeo de `admins` adentro y `EXECUTE` revocado a `PUBLIC`/`anon` (probado: un mayorista logueado recibe "no autorizado").
- **El 25% de pago contado ANTES ERA 8%, y para un cliente dormido eso es argumento de venta.** `acuerdo_parametros.dto_pago_anterior` lo guarda y la RPC devuelve `mejora_pago`: el precio de contado quedó **18,5% mejor** que cuando regía el viejo. Va en la ficha como una línea aparte, porque a un cliente que dejó de comprar hace años se le puede decir que hoy paga bastante menos por lo mismo.
