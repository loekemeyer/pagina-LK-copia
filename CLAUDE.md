...
- **`estadistica_madre` es una VISTA, no una tabla, desde el 2/9/2026.** Antes era una tabla que se llenaba **a mano desde un Excel** (`Y:\AA VENTAS\A7 Estadistica MADRE`) con un importador en Análisis Venta Cliente; la última importación fue el **6/5/2026** (294 filas) y la leían `analisis-venta-cliente.js` **y `script.js`** — o sea, las sugerencias del **portal del cliente** se ordenaban con datos de mayo y 227 productos ni existían ahí. Decisión del usuario: el Excel no se usa más, la proyección sale de `sales_lines`. Ahora es una vista sobre **`estadistica_madre_cache`** (cron diario) con la **misma forma** que la tabla, así que los lectores no cambiaron: `e_madre_uni_mes = proy_uni_mes`, `categoria = familia`, `ranking = row_number() por proy_uni_mes desc`; `tendencia_uni` y `proveedor` salen `NULL` (ninguna pantalla las leía, solo el importador). **Sin `security_invoker` a propósito**: el caché tiene RLS sin policies y la vista debe seguir legible por `anon`/`authenticated` como lo era la tabla. El Excel histórico quedó en **`estadistica_madre_import_20260506`**; el importador se **retiró** de `admin.html` y `analisis-venta-cliente.js` (y del espejo en Virgilio).
- **La regla de descarte de picos de la proyección vive en UNA función: `fn_proy_descarte()`** (2/9/2026, propuesta 2496 de Virgilio). Antes estaba copiada en `_fn_proy_window`, `fn_proyeccion_madre`, `fn_proyeccion_madre_emp`, `refresh_estadistica_madre_cache` y en `admin.js` (x2, LK + espejo), y ya habían divergido. Tenía dos errores que se compensaban: el filtro anulaba por construcción a todo cliente con una sola compra en la ventana (en el 505, 199 de 420 clientes y el 40% de las cajas), y el divisor "meses desde la primera compra" inflaba +51%. Ahora descarta **sólo el excedente** (`v − 1.5×promedio`), **sólo si el cliente compró en ≥ 2 meses**, y el divisor es la **ventana completa**. **Estadística Madre pasó de 24 a 6 meses.** `fn_proyeccion_madre_emp` tenía un hack hardcodeado que metía ventas de Chef sólo para el artículo 505 — era un parche para tapar este mismo bug; se sacó. Definición comentada y backup en el repo Virgilio (`sql/fn_proyeccion_oc_virgilio.sql`, `sql/backups/backup_proyeccion_LK_20260902.sql`).
...
- **Los archivos de `sql/` están al día contra lo desplegado (verificado el 31/7/2026).** Las 25 funciones que documentan coinciden con `pg_proc.prosrc` byte a byte. Igual la fuente de verdad sigue siendo la base: los `.sql` no se ejecutan solos, se corren a mano en el SQL editor, así que un cambio hecho ahí y no volcado los desfasa de nuevo. Para sacar la definición real: `select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = '<nombre>';`. Para verificar un archivo entero contra la base, comparar el md5 del cuerpo normalizado (sin comentarios ni espacios) contra `md5(regexp_replace(regexp_replace(regexp_replace(prosrc,'/\*.*?\*/','','gs'),'--[^\n]*','','g'),'\s','','g'))`.
