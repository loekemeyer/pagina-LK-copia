...
// El archivo local es el que manda y ahora SÍ existe: hasta el 3/8/2026 estaba
// referenciado pero nunca se había commiteado, así que daba 404 y quedaban solo
// los dos CDN. Cuando esos tampoco responden —red de oficina, GitHub bloqueado—
// el mapa caía siempre al fallback simplificado, que son rectángulos y no un
// mapa de Argentina. Es la causa del bug reportado.
//
// Extensión .json y NO .geojson a propósito: IIS devuelve 404 para extensiones
// que no tenga declaradas en staticContent, y el web.config real (que vive solo
// en el servidor) declara .webp/.woff2/.avif pero no .geojson.
//
...
// Proyección del SVG que quedó armado, para poder ubicar puntos (lon/lat) sobre
// el mismo dibujo. Queda en null cuando se usó el fallback simplificado: ese
// SVG tiene coordenadas inventadas a mano, no geográficas, así que proyectar
// contra él pondría los pines en cualquier lado. Quien dibuje puntos tiene que
// chequear que no sea null.
var ARGENTINA_MAP_PROJECTION = null;

// lon/lat → {x, y} en el sistema del viewBox. Devuelve null si no hay proyección.
function arMapProject(lon, lat) {
  var p = ARGENTINA_MAP_PROJECTION;
  if (!p || lon == null || lat == null) return null;
  return {
    x: (lon - p.minLon) / p.lonRange * p.width,
    y: p.height - (lat - p.minLat) / p.latRange * p.height
  };
}
...
function loadArgentinaMapSvg() {
  ...
  ARGENTINA_MAP_LOAD_PROMISE = tryUrl(0)
    ...
    .catch(function (e) {
      ...
      // El fallback no es geográfico: sin proyección, nadie dibuja pines encima.
      ARGENTINA_MAP_PROJECTION = null;
      return ARGENTINA_MAP_SVG;
    });
...
function _buildSvgFromGeoJson(gj) {
  ...
  // Se guarda para que arMapProject() pueda ubicar puntos sueltos (localidades)
  // sobre este mismo dibujo. Sin esto la proyección quedaría encerrada acá y los
  // pines no tendrían cómo alinearse con las provincias.
  ARGENTINA_MAP_PROJECTION = {
