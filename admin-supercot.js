...
(function () {
  ...
  // Edge Function que crea el pedido super en Chef y devuelve el id bigint real.
  // Valida el JWT del admin LK (is_admin) e inserta con service_role server-side.
  var CHEF_CREATE_ORDER_URL =
    "https://nkhzocgdpwtgrmwleihr.functions.supabase.co/create-super-order";
  var CHEF_SHEETS_ENTREGAS_URL =
    "https://nkhzocgdpwtgrmwleihr.functions.supabase.co/sheets-entregas-proxy";

  // ---------------------------------------------------------------------
  // CONFIG DE CADENAS — sale de Supabase (precios_super.cadena vía la RPC
  // get_super_cadenas), NO de este archivo.
  //
  // Hasta el 4/8/2026 eran siete objetos literales acá abajo: agregar una
  // cadena, corregir un cod_cliente o cambiar una condicion de pago exigia
  // editar el JS y desplegar. Ahora es una fila en la tabla. Los objetos
  // siguen existiendo con la MISMA forma y los mismos nombres porque los
  // consumen ~30 lugares del archivo; lo unico que cambio es que nacen
  // vacios y los llena loadSuperCadenas().
  //
  // ⚠ Arrancan VACIOS: todo lo que los lea tiene que correr despues de
  // loadSuperCadenas(). Los dos flujos que los usan (bootstrap y el
  // handleFile de cada card) ya lo esperan.
  // ---------------------------------------------------------------------

  // super_key -> label para pantalla.
  var SUPERS = {};
  ...
  // super_key -> "lk" | "chef": contra que proyecto Supabase va el pedido.
  var SUPER_EMPRESA = {};
  ...
  // participa del import del xlsx. headerRow = primera fila con datos
  // (1-indexed minus 1 in code). Se corta la extraccion al detectar una
  // segunda fila con "Cod" en col0 o 3+ filas vacias.
  // Nota WMart Chef: col 0 = "Cod" (Ref.Prov del PDF y cod en Chef Supabase,
  // ej 769); col 1 = "Cod Loeke" (descriptivo, ej 654). El que usa el PDF es
  // col 0 — por eso hoja_cod_col = 0.
  var SHEET_CONFIG = {};
  ...
  var superCadenasLoaded = false;

  // isChefSuper: super que usa CLIENTE/RPC/SHEETS/L-suffix de Chef.
  // Dorinka y Cencosud van a Chef en estos aspectos (cadena.empresa = 'chef').
  function isChefSuper(superKey) {
    return SUPER_EMPRESA[superKey] === "chef";
  }
  ...
  function usesChefProducts(superKey) {
    return SUPER_USA_PRODUCTOS_CHEF[superKey] === true;
  }

  // Carga la config de cadenas desde Supabase y llena los diccionarios de
  // arriba. Idempotente: la primera llamada gana, las demas salen solas.
  // Si la RPC falla (o el usuario no es admin y devuelve 0 filas) los mapas
  // quedan vacios y el flujo muestra "no configurado" al detectar el PDF, que
  // es el mismo camino que ya existia para una cadena sin entrada.
  async function loadSuperCadenas() {
    if (superCadenasLoaded) return;
    try {
      var r = await window.sb.rpc("get_super_cadenas");
      if (r.error) {
        console.warn("scot: get_super_cadenas error:", r.error.message);
        superCadenasLoaded = true;
        return;
      }
      (r.data || []).forEach(function (c) {
        ...
        if (!k) return;
        SUPERS[k] = c.label || k;
        SUPER_EMPRESA[k] = c.empresa || "lk";
        SUPER_USA_PRODUCTOS_CHEF[k] = c.usa_productos_chef === true;
        if (c.cod_cliente_lk) LK_CUSTOMER_COD[k] = String(c.cod_cliente_lk);
        if (c.cod_cliente_chef) CHEF_CUSTOMER_COD[k] = String(c.cod_cliente_chef);
        if (c.payment_code != null) SUPER_PAYMENT_CODE[k] = Number(c.payment_code);
        // Solo los distintos de la identidad: el resto del archivo hace
        ...
        if (c.hoja_nombre) {
          SHEET_CONFIG[c.hoja_nombre] = {
            key: k,
            codCol: Number(c.hoja_cod_col) || 0,
            priceCol: Number(c.hoja_price_col) || 0,
            dataStartRow: Number(c.hoja_start_row) || 0,
          };
        }
      });
      superCadenasLoaded = true;
    } catch (e) {
      console.warn("scot: loadSuperCadenas error", e);
      superCadenasLoaded = true;
    }
  }
  ...
  // Normalizacion de codigo en JS, espejo de precios_super.norm_cod (SQL):
  // trim -> saca no-alfanumericos del final -> saca ceros a la izquierda -> MAYUS.
  // Hace falta para comparar los codigos del xlsx contra los ya cargados sin
  // falsos "faltantes" por diferencias de grafia (0102 vs 102, "102 " vs 102).
  function normCodJS(s) {
  ...
  // Indice 0-based de columna -> letra de Excel (0->A, 1->B, 26->AA).
  ...
  // ---- Modal generico (promise). Devuelve true (ok) / false (cancel/cerrar).
  // Si opts.info === true muestra un solo boton de cierre y resuelve false.
  function scotModal(innerHTML, opts) {
    opts = opts || {};
    return new Promise(function (resolve) {
      ...
      box.style.cssText =
        "background:var(--surface,#fff);color:var(--text,#1a1a1a);border-radius:12px;" +
        "max-width:" + (opts.maxWidth || 720) + "px;width:100%;max-height:90vh;overflow:auto;" +
        "box-shadow:0 18px 60px rgba(0,0,0,.35);padding:26px 30px";
      var btns = opts.info
        ? '<button data-act="cancel" style="padding:8px 16px;border:1px solid var(--border,#d0d0d0);border-radius:8px;background:var(--surface,#fff);cursor:pointer;font-size:13px">Cerrar</button>'
        : '<button data-act="cancel" style="padding:8px 16px;border:1px solid var(--border,#d0d0d0);border-radius:8px;background:var(--surface,#fff);cursor:pointer;font-size:13px">' +
            (opts.cancelText || "Cancelar") + "</button>" +
          '<button data-act="ok" style="padding:8px 16px;border:0;border-radius:8px;background:#1e8e3e;color:#fff;cursor:pointer;font-size:13px;font-weight:600">' +
            (opts.okText || "Aceptar") + "</button>";
      box.innerHTML =
        innerHTML +
        '<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:18px">' + btns + "</div>";
      ov.appendChild(box);
  ...
  // Dibuja una grilla estilo Excel (gutter con nro de fila + encabezado A/B/C).
  // dataRows: array de filas, cada fila array de celdas. opt.headerRow = indice
  // de la fila que es encabezado (se pinta código azul / precio verde).
  function xlSheet(dataRows, opt) {
    opt = opt || {};
    var gut = "background:var(--surface2,#f0f0f0);color:var(--text3,#888);" +
              "text-align:center;font-size:12px;padding:5px 8px;border:1px solid var(--border,#d7d7d7)";
    var head = '<tr><td style="' + gut + ';min-width:26px"></td>';
    ["A", "B", "C"].forEach(function (L) { head += '<td style="' + gut + ';min-width:104px"><b>' + L + "</b></td>"; });
    head += "</tr>";
    var body = "";
    dataRows.forEach(function (row, ri) {
      var isH = opt.headerRow === ri;
      body += '<tr><td style="' + gut + '">' + (ri + 1) + "</td>";
      for (var c = 0; c < 3; c++) {
        var v = row[c] != null ? row[c] : "";
        var tint = "";
        if (isH && c === 0) tint = "background:#e8f0fe;color:#1a73e8;font-weight:600";
        else if (isH && c === 1) tint = "background:#e6f4ea;color:#1e8e3e;font-weight:600";
        var align = (c === 1 && !isH) ? "text-align:right" : "text-align:left";
        body +=
          '<td style="border:1px solid var(--border,#d7d7d7);padding:6px 12px;font-size:14px;' +
          align + ";" + tint + '">' + v + "</td>";
      }
      body += "</tr>";
    });
    return '<table style="border-collapse:collapse;background:var(--surface,#fff);margin:0 auto">' + head + body + "</table>";
  }

  // Ayuda visual: los DOS formatos aceptados para la lista de un súper, dibujados
  // como una planilla. Se muestra cuando el archivo no trae datos reconocibles.
  function scotShowFormatHelp() {
    var sinEnc = xlSheet([
      ["102E", "2150"], ["103", "1040"], ["104", "1405"], ["106E", "2150"],
    ], {});
    var conEnc = xlSheet([
      ["Codigo", "Precio Uni"], ["102E", "2150"], ["103", "1040"], ["104", "1405"],
    ], { headerRow: 0 });
    var chip = function (t) {
      return '<span style="font-family:monospace;background:var(--surface2,#f0f0f0);padding:1px 6px;border-radius:4px">' + t + "</span>";
    };
    var col = function (titulo, grid, pie) {
      return '<div style="text-align:center">' +
        '<div style="font-weight:600;font-size:16px;margin-bottom:9px">' + titulo + "</div>" +
        grid +
        '<div style="font-size:13px;color:var(--text3,#888);margin-top:9px">' + pie + "</div></div>";
    };
    var html =
      '<div style="text-align:center;min-height:60vh;display:flex;flex-direction:column;justify-content:center">' +
      '<h2 style="margin:0 0 8px;font-size:24px">📄 Cómo tiene que estar el Excel</h2>' +
      '<p style="font-size:16px;color:var(--text3,#888);margin:0 auto 6px;max-width:640px">' +
      "El <b>código</b> va en la <b>columna A</b> y el <b>precio unitario</b> en la <b>columna B</b>. " +
      "Se aceptan los dos formatos:</p>" +
      '<div style="display:flex;justify-content:space-evenly;align-items:flex-start;flex-wrap:wrap;row-gap:24px;margin:22px 0 8px">' +
        col("✔ Sin encabezado", sinEnc, "Datos desde la <b>fila 1</b>.") +
        col("✔ Con encabezado", conEnc, "Fila 1: " + chip("Codigo") + " " + chip("Precio") + ". Datos desde la <b>fila 2</b>.") +
      "</div>" +
      '<div style="background:var(--surface2,#f7f7f7);border-radius:8px;padding:16px 20px;font-size:14px;line-height:1.7;' +
      'margin:22px auto 0;max-width:640px;text-align:left;display:inline-block">' +
      "• La cadena se identifica por el <b>nombre de la hoja</b> (ej. " + chip("DIARCO") + "), no por el nombre del archivo.<br>" +
      "• <b>Un archivo = una lista</b> (una sola hoja con datos).<br>" +
      "• Con encabezado, el código acepta " + chip("Cod") + " / " + chip("Codigo") +
      " y el precio " + chip("Precio") + " / " + chip("Lista Vigente") + "." +
      "</div>" +
      "</div>";
  ...
  async function scotChooseSuper() {
    ...
    // La config se carga en background al abrir la pantalla; asegurarla antes
    // de leerla (si no, un clic temprano abría el selector vacío). Si quedó
    // vacía —p. ej. la primera corrida fue antes de tener sesión de admin—,
    // reintentar una vez forzando la recarga.
    await loadSuperCadenas();
    var keys = keysLK();
    if (!keys.length) {
      superCadenasLoaded = false;
      await loadSuperCadenas();
      keys = keysLK();
    }
    keys.sort(function (a, b) {
      return (SUPERS[a] || a).localeCompare(SUPERS[b] || b, "es");
    });
    if (!keys.length) { alert("No hay cadenas configuradas."); return null; }
    return new Promise(function (resolve) {
      var ov = document.createElement("div");
      ov.style.cssText =
        "position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.45);" +
      ...
      box.style.cssText =
        "background:var(--surface,#fff);color:var(--text,#1a1a1a);border-radius:12px;" +
        "max-width:460px;width:100%;max-height:85vh;overflow:auto;" +
        "box-shadow:0 18px 60px rgba(0,0,0,.35);padding:20px 22px";
      var btns = keys.map(function (k) {
        return (
          '<button data-super="' + k + '" style="display:block;width:100%;text-align:left;' +
          "padding:11px 14px;margin:6px 0;border:1px solid var(--border,#d0d0d0);border-radius:8px;" +
          'background:var(--surface,#fff);cursor:pointer;font-size:14px">' +
          (SUPERS[k] || k) + "</button>"
        );
      }).join("");
      box.innerHTML =
        '<h2 style="margin:0 0 4px;font-size:18px">Elegí el supermercado</h2>' +
        '<p style="font-size:13px;color:var(--text3,#888);margin:0 0 12px">' +
        "Un archivo = una lista. Se cargará la lista de la cadena que elijas.</p>" +
        btns +
        '<div style="display:flex;justify-content:flex-end;margin-top:14px">' +
        '<button data-act="cancel" style="padding:8px 16px;border:1px solid var(--border,#d0d0d0);' +
        'border-radius:8px;background:var(--surface,#fff);cursor:pointer;font-size:13px">Cancelar</button></div>';
      ov.appendChild(box);
      function close(v) { if (ov.parentNode) ov.parentNode.removeChild(ov); resolve(v); }
      ov.addEventListener("click", function (e) {
        ...
        var t = e.target;
        if (t && t.getAttribute) {
          if (t.getAttribute("data-act") === "cancel") return close(null);
          var sk = t.getAttribute("data-super");
          if (sk) return close(sk);
        }
      });
      document.body.appendChild(ov);
    });
  ...
  async function updateSuperPricesFromFile(file, onMsg, onlySuperKey) {
    ...
    try {
      ...
      await loadSuperCadenas(); // asegurar SHEET_CONFIG antes de leerlo
      var buf = await file.arrayBuffer();
      var wb = window.XLSX.read(buf, { type: "array" });
      var listaArr = [], preciosArr = [];
      var fileCodsBySuper = {}; // { super_key: { CODNORM: 1 } } — para detectar faltantes

      // Armar la lista de hojas a procesar: { sheetName, cfg }.
      var targets = [];
      if (onlySuperKey) {
        // Una sola cadena. Buscar su config y la hoja: primero por nombre
        // esperado (ej. DIARCO); si no está, la primera hoja con datos del
        // archivo (un archivo = una lista, aunque venga con otro nombre).
        var cfgName = null, cfgOnly = null;
        Object.keys(SHEET_CONFIG).forEach(function (n) {
          if (SHEET_CONFIG[n].key === onlySuperKey) { cfgName = n; cfgOnly = SHEET_CONFIG[n]; }
        });
        if (!cfgOnly) return msg("Cadena no configurada", "err");
        var useSheet = wb.Sheets[cfgName] ? cfgName : null;
        if (!useSheet) {
          (wb.SheetNames || []).some(function (n) {
            var ws0 = wb.Sheets[n];
            if (!ws0) return false;
            var rr = window.XLSX.utils.sheet_to_json(ws0, { header: 1, defval: "" });
            var hasData = rr.some(function (row) {
              return (row || []).some(function (c) { return c !== "" && c != null; });
            });
            if (hasData) { useSheet = n; return true; }
            return false;
          });
        }
        if (!useSheet) { msg("El archivo no tiene datos", "err"); scotShowFormatHelp(); return; }
      ...
      targets.forEach(function (t) {
        ...
        // Si la hoja no trae fecha en el encabezado (ej. DIARCO, que no tiene
        // fila de encabezados), se usa la fecha de HOY: la lista se está
        // cargando ahora, así que esa es su vigencia.
        if (!fecha) {
          var _hoy = new Date();
          fecha = _hoy.getFullYear() + "-" +
                  String(_hoy.getMonth() + 1).padStart(2, "0") + "-" +
                  String(_hoy.getDate()).padStart(2, "0");
        }
        listaArr.push({ super_key: cfg.key, lista_fecha: fecha });

        // Detección por ENCABEZADO, con las columnas configuradas como fallback.
        // Los índices de columna del Excel se corren seguido (una columna nueva
        // metida en el medio) y ahí cfg.priceCol termina apuntando a "Costo sin
        // aportes" en vez de a la lista: se cargarían COSTOS como precios. Buscar
        // "Lista Vigente"/"Cod" por nombre sobrevive a ese corrimiento. Si no se
        // encuentra el encabezado, se usa la columna configurada (no empeora).
        // Se escanean al menos las 2 primeras filas (no solo hasta dataStartRow):
        ...
        for (var hh = 0; hh < scanHasta; hh++) {
          ...
          for (var cc = 0; cc < hrow.length; cc++) {
            var hv = String(hrow[cc] || "").trim().toLowerCase();
            // Encabezado de codigo: "Cod" exacto (no "Cod Anonima"), o "Codigo".
            if (cC < 0 && (hv === "cod" || hv === "codigo" || hv === "código")) cC = cc;
            // Encabezado de precio: "Lista Vigente"/"Lista a Enviar"/"Lista <n>"
            // (formato super), o "Precio…" unitario/final (formato simple). Se
            // excluye "precio costo"/"costo" para no tomar la columna de costos.
            if (pC < 0 &&
                (hv.indexOf("lista vigente") === 0 ||
                 hv.indexOf("lista a enviar") === 0 ||
                 /^lista\s+\d/.test(hv) ||
        ...
        for (var i = cfg.dataStartRow; i < rows.length; i++) {
          ...
          if (cs === "cod" || cs === "cod anonima" || cs === "codigo" || cs === "código" ||
              cod === "" || cod == null) continue;
          if (typeof price !== "number" || price <= 0) continue;
          var uc = String(cod).trim().toUpperCase();
          if (!uc || seen[uc]) continue;
          seen[uc] = 1;
          preciosArr.push({ super_key: cfg.key, cod: uc, price: price });
          if (!fileCodsBySuper[cfg.key]) fileCodsBySuper[cfg.key] = {};
          fileCodsBySuper[cfg.key][normCodJS(uc)] = 1;
        }
      });
      if (!listaArr.length) {
        msg("El xlsx no tiene hojas de supers LK — mirá el formato esperado", "err");
        scotShowFormatHelp();
        return;
      }

      // ---- Revisión de cambios antes de guardar ----
      // set_super_prices REEMPLAZA la lista de cada cadena (borra e inserta). Se
      // compara el archivo contra lo cargado y se muestra: arriba, cada artículo
      // con precio anterior → nuevo y % de aumento/baja; abajo, separados, los
      // códigos que NO vienen en el archivo (se conservan con su precio anterior).
      await loadSuperPrices();
      // Cargar catálogos para poder mostrar la descripción de cada código.
      try { await loadAllProducts(); } catch (e) {}
      try { await loadAllLokeProducts(); } catch (e) {}
      var dbNorm = {}; // { super: { CODNORM: price } }
      Object.keys(fileCodsBySuper).forEach(function (sk) {
        var dbDict = superListPrices[sk] || {};
        var m = {};
        Object.keys(dbDict).forEach(function (dbCod) { m[normCodJS(dbCod)] = dbDict[dbCod]; });
        dbNorm[sk] = m;
      });
      // Filas del archivo con su comparación.
      var filasBySuper = {}, faltantesBySuper = {};
      var nSube = 0, nBaja = 0, nIgual = 0, nNuevo = 0, sumaPct = 0, nConPct = 0;
      preciosArr.forEach(function (p) {
        var old = (dbNorm[p.super_key] || {})[normCodJS(p.cod)];
        var pct = (old != null && old > 0) ? (p.price - old) / old * 100 : null;
        if (old == null) nNuevo++;
        else if (p.price > old) { nSube++; }
        else if (p.price < old) { nBaja++; }
        else nIgual++;
        if (pct != null) { sumaPct += pct; nConPct++; }
        (filasBySuper[p.super_key] = filasBySuper[p.super_key] || []).push({
          cod: p.cod, old: (old != null ? old : null), nuevo: p.price, pct: pct, isNew: old == null,
        });
      });
      // Faltantes: cargados que no vienen en el archivo.
      var faltantes = [];
      Object.keys(fileCodsBySuper).forEach(function (sk) {
        var dbDict = superListPrices[sk] || {};
        var fileSet = fileCodsBySuper[sk] || {};
        Object.keys(dbDict).forEach(function (dbCod) {
          if (!fileSet[normCodJS(dbCod)]) {
            var f = { super_key: sk, cod: dbCod, price: dbDict[dbCod] };
            faltantes.push(f);
            (faltantesBySuper[sk] = faltantesBySuper[sk] || []).push(f);
          }
        });
      });

      if (!preciosArr.length && !faltantes.length) {
        msg("No se detectaron precios en la hoja — revisá el formato", "err");
        scotShowFormatHelp();
        return;
      }

      // Helpers de formato.
      var money = function (x) { return "$" + Number(x).toLocaleString("es-AR"); };
      var descDe = function (cod, sk) {
        var pinfo = findProductByCod(cod, sk);
        return pinfo && pinfo.product ? (pinfo.product.description || "") : "";
      };
      var pctCell = function (row) {
        if (row.isNew) return '<span style="color:#1a73e8;font-weight:600">nuevo</span>';
        if (row.pct == null) return "—";
        var col = row.pct > 0 ? "#c0392b" : row.pct < 0 ? "#1e8e3e" : "var(--text3,#888)";
        var s = (row.pct > 0 ? "+" : "") + row.pct.toFixed(1).replace(".", ",") + "%";
        return '<span style="color:' + col + ';font-weight:600">' + s + "</span>";
      };

      // Tabla de cambios (arriba), agrupada por cadena.
      var cambiosHtml = "";
      Object.keys(filasBySuper).forEach(function (sk) {
        var label = SUPERS[sk] || sk;
        var filas = filasBySuper[sk].slice().sort(function (a, b) {
          if (a.isNew !== b.isNew) return a.isNew ? 1 : -1; // nuevos al final
          return (b.pct || 0) - (a.pct || 0); // mayor aumento primero
        });
        var body = filas.map(function (r) {
          return (
            '<tr style="border-top:1px solid var(--border,#eee)">' +
            '<td style="padding:4px 8px;font-family:monospace">' + r.cod + "</td>" +
            '<td style="padding:4px 8px">' + (descDe(r.cod, sk) || '<span style="color:var(--text3,#999)">—</span>') + "</td>" +
            '<td style="padding:4px 8px;text-align:right;white-space:nowrap;color:var(--text3,#888)">' + (r.old != null ? money(r.old) : "—") + "</td>" +
            '<td style="padding:4px 8px;text-align:right;white-space:nowrap;font-weight:600">' + money(r.nuevo) + "</td>" +
            '<td style="padding:4px 8px;text-align:right;white-space:nowrap">' + pctCell(r) + "</td></tr>"
          );
        }).join("");
        cambiosHtml +=
          '<div style="font-weight:600;font-size:13px;margin:12px 0 2px">' + label + "</div>" +
          '<table style="width:100%;border-collapse:collapse;font-size:13px">' +
          '<tr style="color:var(--text3,#888);font-size:11px;text-transform:uppercase">' +
          '<td style="padding:2px 8px">Código</td><td style="padding:2px 8px">Descripción</td>' +
          '<td style="padding:2px 8px;text-align:right">Anterior</td>' +
          '<td style="padding:2px 8px;text-align:right">Nuevo</td>' +
          '<td style="padding:2px 8px;text-align:right">Aumento</td></tr>' +
          body + "</table>";
      });

      // Tabla de faltantes (abajo, separada): se conservan con su precio anterior.
      var faltantesHtml = "";
      if (faltantes.length) {
        var listaF = "";
        Object.keys(faltantesBySuper).forEach(function (sk) {
          var label = SUPERS[sk] || sk;
          var body = faltantesBySuper[sk].map(function (f) {
            return (
              '<tr style="border-top:1px solid var(--border,#eee)">' +
              '<td style="padding:4px 8px;font-family:monospace">' + f.cod + "</td>" +
              '<td style="padding:4px 8px">' + (descDe(f.cod, sk) || '<span style="color:#c0392b">(sin ficha de producto)</span>') + "</td>" +
              '<td style="padding:4px 8px;text-align:right;white-space:nowrap">' + money(f.price) + "</td></tr>"
            );
          }).join("");
          listaF +=
            '<div style="font-weight:600;font-size:13px;margin:10px 0 2px">' + label + "</div>" +
            '<table style="width:100%;border-collapse:collapse;font-size:13px">' +
            '<tr style="color:var(--text3,#888);font-size:11px;text-transform:uppercase">' +
            '<td style="padding:2px 8px">Código</td><td style="padding:2px 8px">Descripción</td>' +
            '<td style="padding:2px 8px;text-align:right">Precio (se conserva)</td></tr>' +
            body + "</table>";
        });
        faltantesHtml =
          '<div style="border-top:2px solid var(--border,#ddd);margin-top:18px;padding-top:12px">' +
          '<h3 style="margin:0 0 4px;font-size:15px">No vienen en el archivo — se conservan</h3>' +
          '<p style="font-size:12px;color:var(--text3,#888);margin:0 0 4px">' +
          "Estos códigos ya estaban cargados y el Excel no los trae. Si confirmás, " +
          "se <b>mantienen con su precio anterior</b> (no se borran).</p>" + listaF + "</div>";
      }

      var promedio = nConPct ? (sumaPct / nConPct) : 0;
      var resumen =
        '<div style="font-size:13px;margin:2px 0 6px">' +
        '<span style="color:#c0392b;font-weight:600">▲ ' + nSube + " suben</span> · " +
        '<span style="color:#1e8e3e;font-weight:600">▼ ' + nBaja + " bajan</span> · " +
        "<span style=\"color:var(--text3,#888)\">= " + nIgual + " igual</span> · " +
        '<span style="color:#1a73e8">＋ ' + nNuevo + " nuevos</span>" +
        (nConPct ? ' &nbsp;|&nbsp; promedio <b>' + (promedio > 0 ? "+" : "") + promedio.toFixed(1).replace(".", ",") + "%</b>" : "") +
        "</div>";
      ...
      if (!ok) return msg("Carga cancelada — no se modificó nada", "info");
      faltantes.forEach(function (f) {
        preciosArr.push({ super_key: f.super_key, cod: f.cod, price: f.price });
      });
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      state.rawText = text;
      // La config de cadenas tiene que estar ANTES de resolver el label y el
      // cod_cliente: los loaders de mas abajo corren recien despues del parseo.
      await loadSuperCadenas();
  ...
  function init() {
    ...
    (function () {
      ...
      var pendingSuperKey = null; // cadena elegida en el selector, para el change
      if (btn && fileEl) {
        btn.addEventListener("click", async function () {
          var sk = await scotChooseSuper();
          if (!sk) return; // canceló el selector
          pendingSuperKey = sk;
          setMsg("Cadena: " + (SUPERS[sk] || sk) + " — elegí el archivo…", "info");
          fileEl.click();
        });
        fileEl.addEventListener("change", function () {
          var f = fileEl.files && fileEl.files[0];
          fileEl.value = "";
          var sk = pendingSuperKey;
          pendingSuperKey = null;
          if (f) updateSuperPricesFromFile(f, setMsg, sk);
        });
      }
    })();
    cardInstances = [];
